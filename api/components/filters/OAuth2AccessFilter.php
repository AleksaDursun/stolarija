...
class OAuth2AccessFilter extends \yii\base\ActionFilter
{
    ...
    public function beforeAction($action)
    {
        ...
        if (Yii::$app->hasModule('oauth2')) {
            ...
            $query_param_auth = ['class' => QueryParamAuth::class];
            ...
            $scopes = $actions_scopes[$action_name] ?? '';
            if (is_array($scopes)) {
    ...
    {
        /** @var \api\components\web\BaseApiController $this ->owner */
        ...
        foreach ($access_rules as $rule) {
            if (empty($rule['controllers']) || \in_array($this->owner->uniqueId, $rule['controllers'], true)) {
                ...
                if (!empty($rule['actions']) && \is_array($rule['actions']) && \in_array(
                        $current_action,
                        $rule['actions'],
                        true
                    )
                    ...
                    $is_public = null;
                    if (isset($rule['allow'])) {
                        if ($rule['allow'] && (empty($rule['roles']) || \in_array('?', $rule['roles'], false))) {
