...
namespace common\components\treegrid;

use common\widgets\exportmenu\ExportMenu;
use Yii;
use Closure;
use yii\base\Widget;
...
class TreeGrid extends Widget
{
     ...
     */
    public $options = ['class' => 'table align-items-center table-flush m-0 table-responsive-lg table-hover'];
    ...
    public $pager = [
        ...
        'options' => [
            'class' => 'pagination justify-content-end pt-3 m-0',
        ],
    ...
    public function init()
    {
        ...
        if($this->canBePaginated()){
            ...
            $this->dataProvider->query->andWhere(['IS', $this->parentColumnName, null]);
        }
        ...
        if ($this->enableExport && !$this->exportColumns) {
    ...
    public function run()
    {
        ...
        $exporter = $this->enableExport ? Html::tag('div', ExportMenu::widget([
            'dataProvider' => $this->dataProvider,
            'modelClass' => $this->modelClass,
            'columns' => $this->exportColumns,
            'pjaxContainerId' => $this->pjaxId,
            'columnSelectorOptions' => ['class' => 'btn-sm'],
            'dropdownOptions' => ['class' => 'btn-sm'],
            'container' => ['class' => 'btn-group export-wrapper', 'role' => 'group'],
            'showConfirmAlert' => false,
            'asDropdown' => false,
            'clearBuffers' => true
        ]), $this->exporterBtnOptions) : '';
    ...
    protected function findChildRecords(array $models, $results = []){
        ...
        $parenIds = ArrayHelper::map($models, 'id', 'id');
        $query = clone $this->clonedQuery;
        $children = $query->andWhere(['AND', ['IS NOT', $this->parentColumnName, null], ['IN', $this->parentColumnName, $parenIds]])->all();
    ...
    {
        if (!$this->canBePaginated() || $this->dataProvider->getCount() <= 0) {
            return '';
        }
